...
def sales_contribution_stat(mid):
    ...
    # 服务力：评价得分、投诉意见
    # 退货得分：退货订单数量
    exclude_sid = configure_file.configure_file().get_item("environment variables", "special_shop")
    sql = f""" 
WITH  shop_prm as ( SELECT c.id AS sid,c.mid,f.prm_category_1, f.prm_category_2, d.brand_name, d.area, g.paid_acc, d.beg_date, d.end_date, h.complaint, round(avg(ci.comment_score),2) comment_score, (date_part('day', CURRENT_DATE::TIMESTAMP - min(d.beg_date)::TIMESTAMP ) + 1.00 )::numeric(10,2) online_days  , (date_part('day', max(d.end_date)::TIMESTAMP - min(d.beg_date)::TIMESTAMP ) + 1.00)::numeric(10,2) contract_days     
                    ...
                    WHERE  d.status = 23 AND c.mid = {mid}
                    ...
                    WHERE m.status = 23 AND n.mid = {mid} AND m.contract_no not like '%modify%'
                    ...
                    FROM
                        ...
                        LEFT JOIN (SELECT sid,count(*) pay_people FROM mall{mid}.dwa_user_visit_shop_stat GROUP BY sid) e ON a.sid=e.sid
                        LEFT JOIN (SELECT a.sid,count(a.gid) new_people,sum(a.pay_amount_total) new_pay_amount FROM(
                                            ...
                                            mall{mid}.dwa_user_visit_shop_stat) a WHERE a.row_num = 1 GROUP BY a.sid) f on a.sid=f.sid
                        LEFT JOIN (SELECT mid,sid,
                                    ...
                                    FROM  (SELECT a.mid, A.sid,  (CASE  WHEN b.grade_code is NULL THEN  1 ELSE  b.grade_code  END)  as grade_code,  
                                            SUM ( pay_amount_total )  as pay_amount_total  FROM mall{mid}.dwa_user_visit_shop_stat A  LEFT JOIN public.dwd_user_info b ON A.gid = b.gid  GROUP BY a.mid,A.sid, (CASE  WHEN b.grade_code is NULL THEN  1 ELSE  b.grade_code  END) ) a GROUP BY mid,sid) g ON g.sid=c.id
                        LEFT JOIN ( SELECT mid,sid,sum(order_num) order_num FROM public.dwd_shop_stat_by_day WHERE mid = {mid} GROUP BY mid,sid ) h ON h.sid=a.sid
                    WHERE
                         a.mid = {mid} AND c.is_online = 't' and a.sid {exclude_sid} 
    ...
    shop_data = dbase.select_sql(sql)
...
def prm_category(mid, category):
    sql = f"""SELECT prm_category_{category}  FROM public.dwd_shop_prm_category WHERE mid={mid} GROUP BY mid,prm_category_{category} having count(*) > 1; """
    df = dbase.select_sql(sql)
...
def data_cut(df, colname, boxes, orderby):
    ...
    if orderby == 1:
        ...
        logger.error(f'排序参数错误:{orderby}，正序=1，倒序=2')
    min = df[colname].min()
    max = df[colname].max()
...
def mall_brank_value(mid):
    logger.info(f'mid={mid},starting')
    shop_all = sales_contribution_stat(mid)
    for category_lvl in category_level:
        logger.info(f'mid={mid},category_level={category_lvl}')
        for category in prm_category(mid, category_lvl):
            logger.info(f'mid={mid},category_level={category_lvl},category={category} starting')
            ...
            dbase.execute(delete_data)
            if dbase.insert_to_DB_from_DF(shop_data, 'public.dwa_shop_value', 1000) == 1:
                logger.error(f'插入 public.dwa_shop_value 数据失败，mid= {mid}, category = {category}')
    logger.info(f'mid={mid},数据生成完毕')
...
if __name__ == '__main__':
    ...
    mall_id = public_umall.Mall().mid
    mall_id = [i for i in mall_id if i != '6' and i != '18']
    category_level = [1, 2]
    logger = public.bigdata_logger('brand_value.log')
    dbase = db.database('bigdata_db')

    # test()

    po = Pool(5)  # 定义个进程池
    for i in mall_id:
        po.apply_async(mall_brank_value, (i,), error_callback=public.error_call_back)
        time.sleep(1)
    po.close()
    po.join()

    logger.info(f"Brand value Total duration {datetime.datetime.now() - start_time}")
