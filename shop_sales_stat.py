...
del_shop_stat_sql = """ delete from public.dwd_shop_stat_by_day_copy1 where stat_date between '{}' and '{}'; """
...
def shop_stat(beg_date, end_date, mid):
    ...
    del_sql = del_shop_stat_sql.format(beg_date,end_date)
    del_data = database.select_sql(del_sql,conn)
    logger.info('{}条数据已删除'.format(len(del_data)))

    data = data.fillna(0)
    if common.insert_to_DB_from_DF(data, 'public.dwd_shop_stat_by_day_copy1', 1000) == 1:
        logger.error('插入public.dwd_shop_stat_by_day数据失败')

    logger.info('{}条数据已入库'.format(len(data)))
    return
...
if __name__ == '__main__':
    ...
    logger.info("插入 public.dwd_shop_stat_by_day {0}".format(datetime.datetime.now() - start_time))